    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if touchMovedFirst && isSingleTap {
            if let singleTouch = touches.first{
                ...
                /*sphereNode.position = nodePositionRelativeToQR(nodeDistance: SCNVector3(x: 0.3593571, y: -0.029153042, z: 0.066459715))
                userRootNode.addChildNode(sphereNode)
                
                print(sphereNode.position)
                print(getNodeDistance(nodePosition: sphereNode.position))*/
